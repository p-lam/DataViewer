...
def readBioSignalFile(path):
    ...
    try:
        ...
        date_object = datetime.strptime("2021:"+tstring, "%Y:%H:%M:%S.%f")
...
def displayData(df, df_sa, sr):
    ...
    ax.plot(df_sa["Time"], df_sa["Reaction"].to_numpy()+fNIRS_min[0]-3, linewidth=2, color='purple')
...
def readSustainedFile(path, t0, sr):
    ...
    toTime = lambda s: ((datetime.strptime("2021:"+s, "%Y:%H:%M:%S.%f").timestamp() * 1000 - t0)/1000)
    timeArr = np.array([toTime(s) for s in timeArr])
    data["Time"] = pd.Series(timeArr)
    return data
